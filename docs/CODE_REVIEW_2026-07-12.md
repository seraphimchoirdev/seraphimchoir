...
| A7 | MED | `api/community/albums/quick-upload/route.ts:80` | 월간 자동앨범 find-or-create 레이스 → 📋 부분 유니크 인덱스 (실사용 빈도 낮아 백로그) |
...
| B8 | 서버 | 60s 폴링마다 카드별 getUser+profile 중복 조회 (탭당 분당 2-4회) → 📋 profile 단기 캐시 설계 필요, 효과 대비 리스크 검토 |
| B9 | 서버 | `unstable_cache` 미사용 — 다음 주일 일정/마감 등 전 사용자 공유 데이터를 매 요청 조회 → 📋 태그 무효화 설계와 함께 (잘못 쓰면 낡은 데이터 서빙) |
