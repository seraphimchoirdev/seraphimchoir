...
| 날짜 | 주요 변경사항 |
|------|-------------|
| 2026-09-07 | docs(handoff): 2026-09-07 세션 핸드오프 추가 |
| 2026-09-07 | fix(handoff): 매니페스트를 결정론적으로 생성해 노이즈 diff 제거 |
| 2026-09-07 | refactor(service-schedules): 일괄 등록 미리보기를 읽기 중심 편집으로 전환 |
| 2026-09-07 | feat(service-schedules): 일괄 등록을 전용 페이지로 전환하고 기타 예배 유형 추가 |
| 2026-09-03 | fix(members): 재승격 시 최초 정대원 임명일이 덮이던 문제 수정 |
| 2026-08-18 | feat(members): 신입대원 연습 세트 집계·승격 (5~6단계) |
| 2026-08-18 | feat(members): 신입대원 연습 세트 기록 (1~4단계) |
| 2026-07-29 | fix(service-schedules): 예배 전 연습 시각 누락 보정 및 저장 경로 연결 |
| 2026-07-29 | fix(service-schedules): 주일 2부 예배 외 예배의 예배 후 연습 설정 정합성 복구 |
| 2026-07-28 | docs(task): 조회 불가능한 커밋 해시 204줄 제거 |
| 2026-07-28 | fix(hooks): 문서 자동화 훅이 존재하지 않는 커밋 해시를 기록하던 문제 수정 |
| 2026-07-28 | docs(claude): 자격증명 취급 규칙 및 direnv 구성 문서화 |
| 2026-07-28 | fix(service-schedules): 천안중앙교회 초청예배 시작 시간 19:00으로 보정 |
| 2026-07-28 | fix(service-schedules): 예배 시작 시간이 전부 14:00으로 저장되는 버그 수정 |
| 2026-07-28 | fix(ui): 토스트 타입별 시각적 구분 복구 (성공/에러/경고/정보) |
| 2026-07-28 | feat(service-schedules): 일정·행사를 목록 카드에서 바로 삭제 |
| 2026-07-18 | perf(splash): 커스텀 스플래시 표시 시간 단축 (최대 2.0s → 1.3s) |
| 2026-07-18 | feat(notifications): 좌석 변동 알림 중복 억제 + 보관 7일 자동 정리 + 읽은 알림 지우기 |
| 2026-07-13 | docs(onboarding): 안드로이드 안내에 삼성 인터넷 경로 보강 |
| 2026-07-13 | feat(onboarding): 전대원 오픈 대비 시작 가이드 (앱 페이지 + 인쇄용 PDF) |
| 2026-07-13 | fix(service-schedules): 작곡가·악보출처·후드색상·연습설정이 저장되지 않던 버그 수정 |
| 2026-07-12 | docs(handoff): 2026-07-12 세션 핸드오프 — 리뷰 백로그 완주·v0.3.0 배포·장애 2건 해결 |
| 2026-07-12 | feat(mypage): 알림 차단 시 플랫폼별 재허용 방법 안내 추가 |
| 2026-07-12 | docs(push): Upstash 장애 현황 업데이트 — fail-open 배포로 로그인 복구 |
| 2026-07-12 | feat(navigation): 모바일 하단 네비 개편 — 임원 포털을 더보기로, 배치표를 메인으로 |
...
| 2026-07-12 | feat(push): 관리자 수동 알림 발송 (미투표자 독려 버튼 + 알림 발송 페이지) |
| 2026-07-11 | docs(push): 푸시 알림 배포 체크리스트 작성 |
| 2026-07-11 | feat(push): 배치표·투표 독려 알림 트리거 + pg_cron 스케줄 |
| 2026-07-11 | feat(push): 웹푸시(VAPID) 인프라 + 인앱 알림함 구축 |
...
| 2026-06-23 | feat(community): 사진 이모지 반응을 팝오버(플로팅) 방식으로 변경 |
| 2026-06-23 | feat(community): 앨범 사진 반응·댓글·다운로드 UI + 모바일 삭제 버그 수정 |
| 2026-06-23 | feat(community): 앨범 사진 반응·댓글·다운로드 백엔드 추가 |
