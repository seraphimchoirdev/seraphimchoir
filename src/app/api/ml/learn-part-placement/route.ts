...
function normalizeServiceType(serviceType: string | null): string {
    ...
    // 이미 표준 형식인 경우
    const standardTypes = ['주일 2부 예배', '오후찬양예배', '절기찬양예배', '온세대예배', '기도회', '기타'];
    ...
    if (normalized.includes('2부예배') || normalized.includes('2부 예배') ||
        normalized.includes('2부주일') || normalized.includes('신년예배')) {
        return '주일 2부 예배';
    }
    // 온세대예배 (full 포함)
    ...
    if (normalized.includes('절기') || normalized.includes('특별') ||
        normalized.includes('추수감사') || normalized.includes('부활') ||
    ...
    try {
        ...
        if (source === 'files') {
            ...
            arrangements = arrangementHistories.map(h => {
                ...
                if (rawLayout) {
                    ...
                    const rowCapacities = (rawLayout.rowCapacities as number[]) ||
        ...
        if (learningResult.rules.length > 0) {
            ...
            for (const rule of learningResult.rules) {
                const { error: upsertError } = await (supabase as unknown as { from: (table: string) => { upsert: (data: unknown, options: unknown) => Promise<{ error: { message: string } | null }> } })
