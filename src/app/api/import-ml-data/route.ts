...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { error: scheduleError } = await supabase.from('service_schedules').upsert(
      {
        date: mlData.date,
    ...
    const presentMemberIds = new Set(
      mlData.seats.filter((s) => !s.member_id.startsWith('unknown_')).map((s) => s.member_id)
    );
    ...
    if (existingArrangement) {
      ...
      await supabase.from('arrangements').update({ conductor: '김보미' }).eq('id', arrangementId);

      await supabase.from('seats').delete().eq('arrangement_id', arrangementId);
    } else {
      // 새 arrangement 생성
