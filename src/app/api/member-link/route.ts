...
export async function POST(request: Request) {
  try {
    ...
    if (memberError || !member) {
      return NextResponse.json({ error: '존재하지 않는 대원입니다.' }, { status: 404 });
    }

    // 정대원만 연결 가능
    if (member.member_status !== 'REGULAR') {
      return NextResponse.json({ error: '정대원만 연결할 수 있습니다.' }, { status: 400 });
    }
    ...
    if (existingLink) {
      return NextResponse.json({ error: '이미 다른 계정에 연결된 대원입니다.' }, { status: 400 });
    }
    ...
    if (profile.link_status === 'approved') {
      return NextResponse.json({ error: '이미 대원에 연결되어 있습니다.' }, { status: 400 });
    }

    if (profile.link_status === 'pending') {
      return NextResponse.json({ error: '이미 연결 요청이 대기중입니다.' }, { status: 400 });
    }
    ...
    if (isSinger && height_cm) {
      ...
      if (memberUpdateError) {
        ...
        return NextResponse.json({ error: '대원 정보 업데이트에 실패했습니다.' }, { status: 500 });
      }
    }
