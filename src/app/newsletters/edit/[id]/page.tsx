...
export default function EditNewsletterPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const hasAccess = canEdit || canEditAnnouncements;
    ...
    return (
      <AppShell>
        <div className="container mx-auto max-w-2xl px-4 py-8">
          <Alert variant="error">
            <AlertDescription>소식지 수정 권한이 없습니다.</AlertDescription>
          </Alert>
        </div>
      </AppShell>
    );
      ...
      <div className="container mx-auto max-w-4xl px-4 py-8">
        <div className="mb-6 flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold text-[var(--color-text-primary)]">
            {canEditAnnouncements && !canEdit ? '알림 수정' : '소식지 수정'}
          </h1>
        </div>

        <NewsletterEditor
          newsletterId={id}
          announcementsOnly={canEditAnnouncements && !canEdit}
        />
      </div>
    </AppShell>
  );
