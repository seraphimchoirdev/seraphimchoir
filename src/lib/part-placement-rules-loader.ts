 ...
 */
const DEFAULT_COL_RANGES: Record<
  Part,
  { colRangeByRow: Record<number, ColRange>; avgCol: number }
> = {
  SOPRANO: {
    colRangeByRow: {
...
function getDefaultRules(): Record<Part, PartPlacementRule> {
  ...
  for (const [part, zone] of Object.entries(DEFAULT_PART_ZONES) as [Part, PartZone][]) {
    ...
    const preferredRows = zone.preferredRows.filter((r) => !zone.forbiddenRows.includes(r));
  ...
  try {
    ...
    if (!data || data.length === 0) {
      ...
      if (memberCountRange === '60-69') {
        logger.debug(`[loadPartPlacementRules] 60-69명 샘플 부족, 70-79명 규칙으로 fallback 시도`);
        ...
        if (!fallbackError && fallbackData && fallbackData.length > 0) {
          ...
          const avgConfidence = calculateAverageConfidence(
            fallbackData as LearnedPartPlacementRuleRow[]
          );
          logger.debug(
...
function convertDbRowsToRules(
  ...
  for (const row of rows) {
    ...
    const cleanPreferredRows = (row.preferred_rows || []).filter(
      (r) => !mergedForbiddenRows.includes(r)
    );
    const cleanOverflowRows = (row.overflow_rows || []).filter(
      (r) => !mergedForbiddenRows.includes(r)
    );
 ...
 * - 신뢰도 점수가 임계값 이상
 */
export function isRuleValid(rule: PartPlacementRule, _confidenceThreshold: number = 0.3): boolean {
  // 선호 행이 없으면 유효하지 않음
