 ...
 */
export function analyzePartZonesFromSeats(seats: SeatData[]): Map<Part, PartZoneStatistics> {
...
export function calculateDynamicPartZones(
  ...
  (['SOPRANO', 'ALTO', 'TENOR', 'BASS', 'SPECIAL'] as Part[]).forEach((part) => {
    ...
    // 통계 기반 영역 계산
    const _avgRow = partStats.avgRow; // 추후 확장용
    const avgCol = partStats.avgCol;

    // 측면 결정: 평균 열 위치 기반
    // 중앙을 기준으로 왼쪽(1-7열)이면 'left', 오른쪽(8-15열)이면 'right'
    const side: 'left' | 'right' | 'both' = avgCol <= 7 ? 'left' : avgCol >= 9 ? 'right' : 'both';
    ...
    zones.set(part, {
      ...
      side,
      forbiddenRows,
      preferredRows: preferredRows.length > 0 ? preferredRows : defaultZone.preferredRows,
    });
  });
