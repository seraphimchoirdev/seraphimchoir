...
export function useRestoreDraft(
    ...
    const handleRestoreChoice = useCallback(
        ...
        [arrangementId, loadDraft, deleteDraft, resetWorkflow]
    );

    // Draft 데이터를 store에 적용
    const applyDraft = useCallback(
        (draft: ArrangementDraftData) => {
            ...
            logger.debug(`Applied draft: step=${draft.workflow.currentStep}, assignments=${Object.keys(draft.assignments).length}`);
        },
