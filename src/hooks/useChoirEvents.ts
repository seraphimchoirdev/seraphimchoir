...
export function useChoirEvents(filters?: ChoirEventFilters) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      let query = supabase.from('choir_events').select('*').order('date', { ascending: true });
      ...
      // 분기 필터 (하위 호환성 유지)
        ...
        const { startDate, endDate } = getQuarterDateRange(filters.year, filters.quarter);
        query = query.gte('date', startDate).lte('date', endDate);
      } else {
        // 날짜 범위 필터
...
export function useChoirEvent(id: string | undefined) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const supabase = createClient();
      const { data, error } = await supabase.from('choir_events').select('*').eq('id', id).single();
...
export function useUpdateChoirEvent() {
  ...
  return useMutation({
    mutationFn: async ({ id, data }: { id: string; data: ChoirEventUpdate }) => {
      const supabase = createClient();
...
export function useDeleteChoirEvent() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const supabase = createClient();
      const { error } = await supabase.from('choir_events').delete().eq('id', id);
