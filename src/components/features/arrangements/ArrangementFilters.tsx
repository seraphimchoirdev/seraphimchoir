...
export default function ArrangementFilters({
  ...
  // 활성 필터 개수 계산
  const activeFilterCount = [startDate, endDate, serviceType && serviceType !== 'all'].filter(
    Boolean
  ).length;
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        ...
        <Button variant="outline" onClick={() => setShowFilters(!showFilters)} className="gap-2">
          ...
          <span>필터</span>
          {activeFilterCount > 0 && (
            <span className="ml-1 rounded bg-[var(--color-primary-100)] px-1.5 py-0.5 text-xs font-medium text-[var(--color-primary-700)]">
              {activeFilterCount}
            </span>
          )}
        </Button>
      ...
      {/* 확장 필터 패널 */}
      {showFilters && (
        <Card className="border-[var(--color-border-default)] bg-[var(--color-surface)] p-4">
          <div className="space-y-4">
            {/* 날짜 범위 필터 */}
              ...
              <div>
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    ...
                    >
                      <Calendar className="mr-2 h-4 w-4" />
                      {endDate ? format(endDate, 'yyyy년 M월 d일', { locale: ko }) : '종료일 선택'}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-0" align="start">
                    <CalendarComponent
            ...
            {/* 필터 초기화 버튼 */}
            <div className="flex justify-end border-t border-[var(--color-border-default)] pt-2">
              <Button
                variant="ghost"
                size="sm"
                onClick={onReset}
