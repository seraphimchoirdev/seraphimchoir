...
import { AlertTriangle, ArrowRight, Check, Minus, MoveHorizontal, Plus } from 'lucide-react';
...
const EmergencyChangePreview = memo(function EmergencyChangePreview({
  preview,
  modeLabel: _modeLabel, // 추후 모드별 표시 UI용
  compact = false,
}: EmergencyChangePreviewProps) {
  if (!preview) {
    return <div className="text-muted-foreground text-sm">미리보기를 계산하는 중...</div>;
  }
  ...
  return (
    <div className={cn('space-y-3', compact && 'space-y-2')}>
      ...
      {gridLayoutChanges.rowCapacityChanges.length > 0 && (
        <div className="space-y-1">
          {gridLayoutChanges.rowCapacityChanges.map((change, idx) => (
            <div
              key={idx}
              className={cn('flex items-center gap-2 text-sm', compact ? 'text-xs' : 'text-sm')}
            >
      ...
      <div
        className={cn(
          'border-border flex items-center gap-2 border-t pt-2',
          compact ? 'pt-1.5' : 'pt-2'
        )}
      >
        {movedMemberCount === 0 ? (
          <>
            ...
            <span className={cn('font-medium text-emerald-600', compact ? 'text-xs' : 'text-sm')}>
              이동 인원: 0명
            </span>
          </>
        ) : (
          <>
            ...
            <span className={cn('font-medium text-amber-600', compact ? 'text-xs' : 'text-sm')}>
...
const CascadeChangeItem = memo(function CascadeChangeItem({
  ...
  return (
    <div className={cn('flex items-start gap-2', compact ? 'text-xs' : 'text-sm')}>
      ...
      <span className={getTextColor()}>{change.description}</span>
    </div>
  );
});
