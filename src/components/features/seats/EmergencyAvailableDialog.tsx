...
import { UserPlus, Search, Check, MousePointer, Zap } from 'lucide-react';
import {
    Dialog,
    DialogContent,
    DialogHeader,
...
import type { EmergencyAvailableDialogProps, AvailableProcessMode, EmergencyChangePreview as PreviewType } from '@/types/emergency-changes';
...
const EmergencyAvailableDialog = memo(function EmergencyAvailableDialog({
    ...
    const getPartColor = (part: Part) => {
        switch (part) {
            case 'SOPRANO': return 'bg-pink-100 text-pink-700 border-pink-300';
            case 'ALTO': return 'bg-orange-100 text-orange-700 border-orange-300';
            case 'TENOR': return 'bg-blue-100 text-blue-700 border-blue-300';
            case 'BASS': return 'bg-emerald-100 text-emerald-700 border-emerald-300';
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[500px]">
                            ...
                            <div className="p-2 space-y-1">
                                {filteredMembers.map((member) => (
                                    ...
                                    >
                                        ...
                                        <Badge
                                            variant="outline"
                                            className={cn('text-xs', getPartColor(member.part))}
                                        >
                ...
                {selectedMember && (
                    <div className="space-y-2">
                        <h4 className="text-sm font-medium text-muted-foreground">처리 방식</h4>
                        <div className="grid grid-cols-2 gap-2">
                            ...
                            >
                                ...
                                <Zap className="h-6 w-6 text-emerald-600 mb-1" />
                            ...
                            >
                                ...
                                <MousePointer className="h-6 w-6 text-muted-foreground mb-1" />
                ...
                {selectedMember && processMode === 'AUTO_PLACE' && (
                    <div className="space-y-2">
                        <h4 className="text-sm font-medium text-muted-foreground">자동 배치 미리보기</h4>
                ...
                <DialogFooter>
                    ...
                    <Button
                        onClick={handleApply}
                        disabled={!selectedMember || isLoading || (processMode === 'AUTO_PLACE' && !autoPlacePreview)}
