...
export default function QuarterSelector({ year, quarter, onChange }: QuarterSelectorProps) {
  const handlePrev = () => {
    if (quarter === 1) {
      onChange(year - 1, 4);
    } else {
  ...
  return (
    <div className="flex items-center gap-2 sm:gap-4">
      <Button variant="outline" size="icon" onClick={handlePrev} aria-label="이전 분기">
      ...
      <Button variant="outline" size="icon" onClick={handleNext} aria-label="다음 분기">
