...
import { type RowLeaderCandidate, selectRowLeaders } from '@/lib/row-leader-utils';
...
export interface AutoPlacementResult {
  /** 배치 성공한 멤버 */
  placed: Array<
    UnassignedMember & { newPosition: { row: number; col: number }; searchLevel: string }
  >;
  /** 여전히 미배치인 멤버 */
...
const saveToHistory = (state: ArrangementState): HistoryManager => ({
  past: [...state._history.past, { assignments: { ...state.assignments } }].slice(
    -MAX_HISTORY_SIZE
  ),
...
export const useArrangementStore = create<ArrangementState>((set, get) => ({
  ...
  handleSeatClick: (row, col) => {
    ...
    if (state.selectedSource === 'grid') {
      ...
      if (seatKey === selectedSeatKey) {
        ...
        state.moveMember(state.selectedPosition!.row, state.selectedPosition!.col, row, col);
        state.clearSelection();
      }
    }
  },
  ...
  applyMinimalReassignment: (removedPosition, newLayout, partZones, preferences) => {
    ...
    const zones =
      partZones ||
      new Map(
        (['SOPRANO', 'ALTO', 'TENOR', 'BASS', 'SPECIAL'] as Part[]).map((part) => [
          part,
          DEFAULT_PART_ZONES[part],
        ])
      );
      ...
      const targetPos = member.originalPosition || {
        ...
        col:
          zone.side === 'left'
            ? 3
            : zone.side === 'right'
              ? gridLayout.rowCapacities[0] - 2
              : Math.floor(gridLayout.rowCapacities[0] / 2),
      };
  ...
  shrinkRowFromSide: (row, side) =>
    set((state) => {
      ...
      if (side === 'right') {
        ...
        logger.debug(
          `${row}행 오른쪽 축소: ${newCapacities[rowIndex] + 1}→${newCapacities[rowIndex]}`
        );
      } else {
        ...
        newCapacities[rowIndex]--;
        logger.debug(
          `${row}행 왼쪽 파트 축소 완료: ${newCapacities[rowIndex] + 1}→${newCapacities[rowIndex]}`
        );
      }
  ...
  crossRowFillFromBack: (frontRow, emptyCol, part) => {
    ...
    logger.debug(
      `${candidateMember.memberName}(${part}) ${backRow}행${candidateCol}열 → ${frontRow}행${emptyCol}열`
    );
  ...
  shouldCrossRowMove: (frontRow, part, threshold = 2) => {
    ...
    if (shouldMove) {
      logger.debug(
        `불균형 감지: ${frontRow}행(${frontRowCount}명) vs ${backRow}행(${backRowCount}명), 차이: ${backRowCount - frontRowCount}명`
      );
    }
  ...
  simulateAutoPlace: (memberId, memberName, part) => {
    ...
    for (let r = zone.allowedRows[0]; r <= zone.allowedRows[1] && r <= gridLayout.rows; r++) {
      const rowMemberCount = Object.values(newAssignments).filter(
        (a) => a.row === r && a.part === part
      ).length;
